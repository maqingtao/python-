...
from messageStore.storeMessage import storeMessage
...
class P2pChat(tk.Frame):
    ...
    def __init__(self, master=None,username=None):
        ...
        if messagebox.askyesno("", "Are you hosting the chat room?"):
            self.chat = server.Server(DEFAULT_PORT,username)
    ...
    # def prompt_new_name(self,username):
    #     if username is not None:
    #         self.request_name_change(username)
    #
    ...
    def send_msg(self, event=None):
        if self.chat is not None:
            ...
            storeMessage().store(msg,myaddr,self.ip_entry.get())
